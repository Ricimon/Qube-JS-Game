...
// shaders.js:  Fill in this file with subclasses of Shader, each of which will store and manage a complete GPU program.
  ...
  { 'update_uniforms'          : function( g_state, model_transform, material )     // Send javascrpt's variables to the GPU to update its overall state.
      {
          ...
          inv_trans_CM   = toMat3( transpose( inverse( CM ) ) );        // we'll need of the three special matrices and just send those, since these
          ...
          void main()
          {
            ...
            if( GOURAUD )         // Gouraud mode?  If so, finalize the whole color calculation here in the vertex shader, one per vertex, before we even break it down to pixels in the fragment shader.
            {
              ...
              for(int i = 0; i < N_LIGHTS; i++)
              {
                ...
                float diffuse  = max( dot(N, L[i]), 0.0 );							// Max is used because dot products can turn out negative, and negative lighting doesn't make sense
    ...
    'fragment_glsl_code_string': function()           // ********* FRAGMENT SHADER *********
      { return `
          ...
          void main()
          {
            ...
            {
              float attenuation_multiplier = 1.0 / (1.0 + attenuation_factor[i] * (dist[i] * dist[i]));
			  float diffuse  = max( dot(N, L[i]), 0.0 );						// Same calculations as in vertex shader
              float specular = pow( max( dot(N, H[i]), 0.0 ), smoothness );
