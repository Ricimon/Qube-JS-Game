...
var viewSize = 20, currentScene = 0, seePickingColors = false, earthquake_shake = 0, cameraRotating = 0;
var rumble = new Audio('Earth Rumble Sound Effect.mp3'), level_transition = new Audio('level_transition.mp3');
      ...
      { this.define_data_members( { filename, bool_mipMap, bool_will_copy_to_GPU,       id: gl.createTexture() } );
        ...
        this.image.onload   = ( function (texture, bool_mipMap)       // This self-executing anonymous function makes the real onload() function
          { return function( )      // Instructions for whenever the real image file is ready
            { gl.pixelStorei  ( gl.UNPACK_FLIP_Y_WEBGL, true );	// Specify pixel storage modes
              gl.bindTexture  ( gl.TEXTURE_2D, texture.id );
              gl.texImage2D   ( gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, texture.image );
              gl.texParameteri( gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR );	// Set parameter for rendering close-up
			  // Set parameters for rendering far away, either using mipmap or not
              if( bool_mipMap )
